 ...
 *
 * Steps:
 *   1. DemoApp1 loads in default "grid" layout (4 columns)
 *   2. Dashboard chat (port 4000): "Create product-card-layout flag with value large-image"
 *   3. AI calls create_feature tool → Exp Engine stores the flag
 *   4. DemoApp1 reloads → Exp Engine SDK picks up the flag
...
test.describe("Demo 1: Feature Flag → Live Layout Change", () => {
  ...
  test("AI creates feature flag → product layout changes from grid to large-image", async ({
    ...
    // ── Step 1: DemoApp loads with default "grid" layout ──────────────────
    await test.step("Step 1 — DemoApp1 starts in grid layout (default)", async () => {
      ...
      console.log("\n  📸 01a — DemoApp1: grid layout (before)");
    ...
    // ── Step 3: Reload → Exp Engine SDK picks up the new flag ──────────────
    await test.step("Step 3 — Reload page → large-image layout active", async () => {
      ...
      // Exp Engine SDK init() is async — badge re-renders when features arrive (allow up to 12s)
      const badge = page.locator("span.uppercase.bg-indigo-50");
      ...
      console.log('\n  📸 01c — DemoApp1: large-image layout (after)');
      console.log(`     Layout badge: "${await badge.textContent()}"`);
    });
