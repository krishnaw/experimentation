 ...
 *
 * Mirrors the SafewayDeals providers.py pattern:
 *   - Free providers (Cerebras → Mistral → Groq) tried in order
 *   - Module-level rate-limit map with 10-min backoff per provider/model
 *   - Anthropic kept as paid last-resort with sliding-window usage limits
 */
...
export function buildFallbackChain(env: Record<string, string | undefined> = process.env): ChainEntry[] {
  ...
  if (env.CEREBRAS_API_KEY) {
    const c = createOpenAI({ baseURL: "https://api.cerebras.ai/v1", apiKey: env.CEREBRAS_API_KEY });
    chain.push({ key: "cerebras/llama-3.3-70b", name: "cerebras/llama-3.3-70b", model: c.chat("llama-3.3-70b") });
  }

  if (env.MISTRAL_API_KEY) {
    const c = createOpenAI({ baseURL: "https://api.mistral.ai/v1", apiKey: env.MISTRAL_API_KEY });
  ...
  if (env.GROQ_API_KEY) {
    const c = createOpenAI({ baseURL: "https://api.groq.com/openai/v1", apiKey: env.GROQ_API_KEY });
