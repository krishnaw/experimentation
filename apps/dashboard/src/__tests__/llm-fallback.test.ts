 ...
 *
 * Covers:
 *  1. buildFallbackChain — respects env vars, correct order, Anthropic excluded
 *  2. Rate-limit tracking — mark, skip, expiry, clear
 *  3. Anthropic sliding-window guard — burst/hourly/daily limits
 *  4. HTTP integration — X-Model header from live :4000 server (uses Mistral since
 *     only MISTRAL_API_KEY is configured)
 */
...
describe("rate-limit tracking", () => {
  ...
  it("markRateLimited with 0ms expires immediately on next check", () => {
    ...
    // Allow event loop to tick so Date.now() >= expiry
  ...
  it("multiple calls to markRateLimited reset the expiry", () => {
    ...
    markRateLimited("groq/llama-3.3-70b-versatile", 60_000); // extend
    expect(isRateLimited("groq/llama-3.3-70b-versatile")).toBe(true);
  });
});

// ── 3. Anthropic sliding-window usage guard ───────────────────────────────────
describe("Anthropic usage guard", () => {
  ...
  it("sliding window prunes entries older than the largest window", () => {
    // Simulate usage that happened 25 hours ago by mutating the timestamp
    vi.useFakeTimers();
    const now = Date.now();
    vi.setSystemTime(now - 25 * 60 * 60_000); // 25h ago
    recordAnthropicUsage();
    vi.setSystemTime(now); // restore
    // The 25h-old entry should be pruned, so isAnthropicAllowed still true
    expect(isAnthropicAllowed()).toBe(true);
...
// ── 4. HTTP integration — live :4000 server ───────────────────────────────────
describe.skip("HTTP integration — live /api/chat (requires dashboard on :4000)", () => {
  const DASHBOARD = "http://localhost:4000";
  const MSG = [{ role: "user", content: "Say the word OK and nothing else." }];

  // Single real LLM call — verify status, X-Model header, body, and provider all at once
  it("responds 200 with X-Model header, non-empty body, and a free provider (not anthropic)", async () => {
    ...
    // Matches a known provider pattern
    const knownPatterns = [/^cerebras\//, /^mistral\//, /^groq\//, /^anthropic\//];
    expect(knownPatterns.some((p) => p.test(model))).toBe(true);

    // Body is non-empty text
    const text = await res.text();
    expect(text.length).toBeGreaterThan(0);

    // Free provider was selected (Mistral key is configured, Anthropic should not be primary)
